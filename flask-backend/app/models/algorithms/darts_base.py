...
class DartsForecaster(ForecasterBase):
    def __init__(
        ...
        :param weather_agent: An object that provides weather data.
        :param use_time_features: Whether to use time features.
        :param use_weather_features: Whether to use weather features.
        """
        ...
        should_use_covs = self.use_time_features or self.use_weather_features
    ...
    def get_measurements(self, new_col_label: str = "consumption"):
        ...
        """
        # TODO: Check if diff should be true or not
        df_measurements = data_controller.get_measurements(
            self.meter_id, aggregate=True, diff=True, max_missing=0.01
        )
    ...
    def get_covariates(self, df_measurements: pd.DataFrame, future_steps=0):
        ...
        end_date = df_measurements.index[-1] + pd.Timedelta(
            hours=future_steps + self.forecast_horizon
        )
        ...
        if self.use_weather_features:
            # TODO: get future weather, but distinguish different scenarios
            raise NotImplementedError("Weather features are not implemented yet")
    ...
    def get_num_hours_to_predict(
        ...
        if forecast_day_end < last_measurement_date:
            raise BadRequestException(
                f'Forecast date "{forecast_date.isoformat()}" must be in the future'
    ...
    def find_best_hyperparams(self, datasets, static_model_params={}):
        """
    ...
    def train_with_tune_parameters(self, model_args, datasets, static_model_params):
        ...
        """
        model = self.Algorithm(**model_args, **static_model_params)
        model.fit(**datasets)
    ...
    def get_train_datasets(self, datasets, with_val=False):
        """
        ...
        :param with_val: If True, the validation set is added to the training set.
        ...
        if self.has_covariates and needs_val_set:
    ...
    def fit(self, train_size: float = 0.8, hyper_opt: bool = False):
        ...
        if hyper_opt:
            ...
            best_hyperparams = self.find_best_hyperparams(
                hyperopt_datasets, static_model_params=static_params,
            )
        ...
        self.model = self.Algorithm(**tunable_params, **static_params)
