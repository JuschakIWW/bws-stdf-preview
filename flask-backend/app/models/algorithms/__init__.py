...
def get_model(meter_id, algorithm: str):
    ...
    if algorithm == "xgboost":
        return XGBForecaster(meter_id)
    elif algorithm == "tripleexponentialsmoothing":
        return TESForecaster(meter_id)
    elif algorithm == "autoarima":
        return AutoArimaForecaster(meter_id)
    elif algorithm == "nbeats":
        return NBEATSForecaster(meter_id)
    else:
...
def get_all_algorithms(include_params: bool = False):
    ...
    :param include_params: If True, the specifications will include the parameters of each algorithm.
    :return: A list of dictionaries, each containing the name, description and parameters of an algorithm.
    """
    algs = [XGBForecaster, TESForecaster, AutoArimaForecaster, NBEATSForecaster]
