...
from app.schemas import (
    ...
    TrainOptions as TrainOptionsSchema,
)
...
def raise_if_model_not_exists(meter_id: str, algorithm: str):
    ...
    model = client[DB_NAME][MODEL_COLCTN].find_one(
        {"refMeter": meter_id, "algorithm": algorithm}
    )
    if model is None:
...
def get_pm_measurements(pmid, check_exists=False, as_df=False, compact=False):
    ...
    ), "Converting between different measurement units is not supported yet."
...
def get_virtual_meter(vmid: str):
    ...
    query = {"_id": vmid}
    mask = {"_id": 0}
    return client[DB_NAME][VM_COLCTN].find_one(query, mask)
...
def generate_vm_id():
    ...
    All ids are generated in ascending order.
    """
    client = get_mongo_client()
    vmids = list(client[DB_NAME][VM_COLCTN].distinct("_id"))
    vmids = sorted([int(vmid.split(":")[-1]) for vmid in vmids])
    vmid = vmids[-1] + 1 if len(vmids) > 0 else 1
...
def invalidate_models(vmid: str, algorithm: str = None):
    ...
    if algorithm:
        ...
        filter = {"refMeter": vmid, "algorithm": algorithm}
...
def set_default_model(meter_id: str, algorithm: str):
    ...
    query = {"refMeter": meter_id, "algorithm": algorithm}
    update = {"$set": {"isDefault": True}}
    client[DB_NAME][MODEL_COLCTN].update_one(query, update)


def update_or_create_model_meta(properties, setDefault=False):
    """
...
def save_model(model, meter_id: str, algorithm: str, results: dict):
    """
    ...
    try:
        ...
        update_or_create_model_meta(model_meta)
        return model_id
    except:
        # Clean up
...
def get_meters_contained(vmid):
    """
    Returns a list of all virtual and physical meters that are submeters of the given virtual meter.
    """
    meters = []
    if is_virtual(vmid):
        vm = get_virtual_meter(vmid)
        for submeter in vm["submeterIds"]:
            ...
            if is_virtual(submeter):
                meters.extend(get_meters_contained(submeter))
    return meters
...
def raise_if_not_disjoint(meter_ids):
    ...
    for mid in meter_ids:
        submeters.append(mid)
        submeters.extend(get_meters_contained(mid))
...
def is_virtual(meter_id: str):
    ...
    """
    virtual_pattern = re.compile(r"^urn:ngsi-ld:(virtualMeter):[a-zA-Z0-9]+$")
    physical_pattern = re.compile(r"^urn:ngsi-ld:(Device):[a-zA-Z0-9]+$")
...
def parse_train_options(data: dict, algorithm: str = None):
    """
    ...
    :param data: dict of raw request parameters that contains the training options.
                 properties that are not part of TrainOptionsSchema are ignored.
    :param algorithm: The algorithm that should be used for training.
    """
    train_options = TrainOptionsSchema().load(data, unknown=EXCLUDE)
    if algorithm:
...
def get_framework(algorithm):
    ...
    if algorithm in ["xgboost", "tripleexponentialsmoothing", "autoarima", "nbeats"]:
        return "darts"
    else:
